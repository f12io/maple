...
| `float`           | Gentle vertical float       | `--float-distance` (default: `8px`)                                                                                                                                                                                                                                                                                                                                                                                                                  | `anim-float_2500_ease-in-out_infinite`               |
| `border-beam`     | Animated border beam        | `--beam-offset` (default: `0px`)<br>`--beam-radius` (default: `inherit`)<br>`--beam-width` (default: `1px`)<br>`--beam-opacity` (default: `1`)<br>`--beam-core-color` (default: `currentColor`)<br>`--beam-glow-color` (default: `color-mix(in oklch, currentColor 75%, transparent)`)<br>`--beam-tail-start` (default: `75%`)<br>`--beam-glow-start` (default: `88%`)<br>`--beam-core-start` (default: `92%`)<br>`--beam-glow-end` (default: `96%`) | `anim-border-beam_2500_linear_infinite`              |
...
| `wiggle`          | Rotational wiggle           | `--wiggle-angle` (default: `10deg`)                                                                                                                                                                                                                                                                                                                                                                                                                  | `anim-wiggle_400_ease-in-out`                        |
...
By default, Maple resolves conflicts between utility classes at runtime. If an element has multiple classes targeting the same property (e.g., `p-4` and `p-8`), Maple determines the winner based on the order (last one wins) and removes the overridden classes from the DOM. This ensures a clean DOM and predictable styling that matches the class string order.
...
2.  **Performance Boost**: Skips the overhead of conflict calculation and DOM manipulation, resulting in faster initial rendering.
