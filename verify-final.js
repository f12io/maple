...
import maple, { loadPrecalculatedProperties } from './src/engines/maple/property-extraction.js';
...
async function run() {
    ...
    console.log('Checking map access (should be undefined):', maple.properties.shortMap?.bg);

    console.log('Loading pre-calculated properties...');
    await loadPrecalculatedProperties();

    const props = maple.properties;
    console.log('Properties count after loading:', Object.keys(props.shortMap).length);
